...
fn layers_from_precompile_cache(ctx: &impl RuntimeContext) -> bool {
    match ctx.entrypoint().source {
        containerd_shim_wasm::sandbox::context::Source::Oci(layers) => {
            let wasm_layers: Vec<_> = layers.iter().filter(|l| is_wasm_content(l).is_some()).collect();
            ...
            .map(|layer| match is_wasm_content(layer) {
                Some(wasm_layer) => {
                    ...
                    if wasm_layer.precompiled {
                        ...
                        // untrusted image layers (GHSA-cc25-vq59-rcjx).
                        let component =
...
mod tests {
    ...
    /// GHSA-cc25-vq59-rcjx: an OCI image layer containing valid precompiled
    ...
    async fn precompile_rejects_oci_layer_with_precompiled_bytes() {
        ...
        assert!(
            result.is_err(),
            "OCI layer with precompiled bytes must not be trusted (GHSA-cc25-vq59-rcjx)"
        );
    }
}
